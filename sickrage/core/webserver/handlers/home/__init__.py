...
class TestTorrentHandler(BaseHandler):
    def get(self, *args, **kwargs):
        torrent_method = clean_url(self.get_query_argument('torrent_method'))
        host = clean_url(self.get_query_argument('host'))
        username = self.get_query_argument('username')
        password = self.get_query_argument('password')
...
class TestNMAHandler(BaseHandler):
    def get(self, *args, **kwargs):
        ...
        nma_priority = self.get_query_argument('nma_priority', 0)
...
class RestartHandler(BaseHandler):
    def get(self, *args, **kwargs):
        pid = self.get_query_argument('pid')
        force = self.get_query_argument('force')
...
class EditShowHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        exceptions_list = self.get_body_argument('exceptions_list', None)
        flatten_folders = self.get_body_argument('flatten_folders', None)
        paused = self.get_body_argument('paused', None)
        direct_call = self.get_body_argument('directCall', None)
...
class DeleteShowHandler(BaseHandler):
    def get(self, *args, **kwargs):
        show = self.get_query_argument('show')
        full = self.get_query_argument('full', 0)
...
class SearchEpisodeHandler(BaseHandler):
    def get(self, *args, **kwargs):
        ...
        down_cur_quality = self.get_query_argument('downCurQuality', 0)
