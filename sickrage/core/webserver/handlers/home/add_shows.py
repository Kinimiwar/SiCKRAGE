...
class NewShowHandler(BaseHandler):
    def get(self, *args, **kwargs):
        ...
        if not isinstance(other_shows, list):
...
class AddShowByIDHandler(BaseHandler):
    async def get(self, *args, **kwargs):
        ...
        return self.write(await AsyncHTTPClient().fetch(
            url_concat("/home/addShows/newShow", {'show_to_add': '1|{show_dir}|{indexer_id}|{show_name}'.format(**{
                'show_dir': show_dir,
...
class AddNewShowHandler(BaseHandler):
    async def post(self, *args, **kwargs):
        ...
        rootDir = self.get_body_argument('rootDir')
        ...
        anyQualities = self.get_body_argument('anyQualities', None)
        bestQualities = self.get_body_argument('bestQualities', None)
        ...
        other_shows = self.get_body_argument('other_shows', None)
        ...
        async def finish_add_show():
            ...
            return self.write(await AsyncHTTPClient().fetch(
                url_concat("/home/addShows/newShow", {'show_to_add': next_show_dir, 'other_shows': rest_of_show_dirs})))
...
class AddExistingShowsHandler(BaseHandler):
    async def post(self, *args, **kwargs):
        ...
        """
        shows_to_add = self.get_body_argument('shows_to_add', [])
        prompt_for_settings = self.get_body_argument('promptForSettings')

        # grab a list of other shows to add, if provided
        if not isinstance(shows_to_add, list):
        ...
        if prompt_for_settings and shows_to_add:
            return self.write(await AsyncHTTPClient().fetch(url_concat("/home/addShows/newShow",
        ...
        return self.write(await AsyncHTTPClient().fetch(
            url_concat("/home/addShows/newShow", {'show_to_add': dirs_only[0], 'other_shows': dirs_only[1:]})))
